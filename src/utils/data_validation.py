...
class DataValidator:
    ...
    def validate_dataframe(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        logger.info(f"Validation completed: {validation_report['valid_records']}/{validation_report['total_records']} valid records")
        logger.info(f"Data quality score: {validation_report['data_quality_score']:.2f}%")
        
        return cleaned_df, validation_report
    
    def _validate_required_columns(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    def _validate_directions(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
        ...
        if 'sentido' in df.columns:
            invalid_directions = df[~df['sentido'].isin(valid_directions)]['sentido'].unique()
        ...
        for col in date_columns:
            if col in df.columns:
                ...
                if len(invalid_dates) > 0:
                    errors.append(f"Invalid dates in column '{col}': {len(invalid_dates)} records")
                ...
                if len(future_dates) > 0:
                    errors.append(f"Future dates in column '{col}': {len(future_dates)} records")
...
"""
    ...
    def validate_incremental_data(self, new_df: pd.DataFrame, existing_df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
        ...
        """
        logger.info(f"Validating incremental data: {len(new_df)} new records against {len(existing_df)} existing records")
        ...
        if not existing_df.empty and not new_df.empty:
            ...
            if len(duplicates) > 0:
                ...
                validation_report['validation_errors'].append(f"Found {len(duplicates)} duplicate records")
        ...
        validation_report['validation_errors'].extend(standard_report['validation_errors'])
        
        logger.info(f"Incremental validation completed: {validation_report['valid_new_records']} valid new records")
