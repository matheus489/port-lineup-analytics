...
class DailyScheduler:
    ...
    def run_daily_collection(self):
        ...
        try:
            ...
            paranagua_data = self.paranagua_collector.collect_data(start_date_str, end_date_str)
            ...
            santos_data = self.santos_collector.collect_data(start_date_str, end_date_str)
    ...
    def _process_source_data(self, df, source: str, collection_date: str) -> Dict[str, Any]:
        """Process data for a single source through all medallion layers"""
        logger.info(f"Processing {len(df)} records from {source}")
        
        try:
            # Bronze layer
            bronze_file = self.pipeline.process_bronze_layer(df, source, collection_date)
    ...
    def run_incremental_update(self):
        ...
        try:
            ...
            if paranagua_start <= today:
                logger.info(f"Collecting incremental data from Paranaguá from {paranagua_start}")
                paranagua_data = self.paranagua_collector.collect_data(
                    paranagua_start.strftime('%Y-%m-%d'),
                    today.strftime('%Y-%m-%d')
                )
                if not paranagua_data.empty:
                    self._process_source_data(paranagua_data, 'paranagua', today.strftime('%Y-%m-%d'))
            
            if santos_start <= today:
                logger.info(f"Collecting incremental data from Santos from {santos_start}")
                santos_data = self.santos_collector.collect_data(
                    santos_start.strftime('%Y-%m-%d'),
                    today.strftime('%Y-%m-%d')
                )
                if not santos_data.empty:
                    self._process_source_data(santos_data, 'santos', today.strftime('%Y-%m-%d'))
    ...
    def run_data_cleanup(self):
        ...
        try:
            ...
            if not quality_report.empty:
                quality_file = Config.BASE_DATA_PATH / f"quality_report_{datetime.now().strftime('%Y%m%d')}.csv"
